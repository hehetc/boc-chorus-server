...
module.exports = {
    ...
    getInfo:function (activityId,callback) {
        var sql = 'SELECT * FROM v_activity_info WHERE F_ActivityId = ?'; //v_activity_info查活动本身信息，联合用户昵称
    ...
    haveChecked:function (activity_id,user_id,callback) {
        var sql = 'SELECT * FROM t_check WHERE F_UserId = ? AND F_ActivityId = ?';
        var params = [user_id,activity_id];
        db.connection.query(sql,params,function (err,res) {
            if(err){
                console.log(err);
